...
contract BookMarketplace is ErrorMessages {
    ...
    function purchaseBook(
        ...
        // this checks if the msg.sender already bought this
    ...
    // maybe client was refunded but then repurchased
    function repurchaseBook(
        bytes32 bookHash
    ) external payable onlyWhenNotStopped {
        if (!isBookCreated(bookHash)) {
            revert BookIsNotCreated();
